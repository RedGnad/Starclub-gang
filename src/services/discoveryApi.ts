...
export async function syncDApps(progressCallback?: (current: number, total: number) => void): Promise<DApp[]> {
  try {
    ...
    const enrichedDapps = await enrichTwitterData(dapps);

    // Sort by quality score descending
    enrichedDapps.sort((a, b) => b.qualityScore - a.qualityScore);

    console.log(`✅ Successfully synchronized ${enrichedDapps.length} dApps`);
